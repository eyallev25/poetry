...
def wait_for_job_to_complete(name: str, namespace: str):
    ...
    for event in w.stream(batchapi_client.list_namespaced_job, namespace=namespace):
        ...
        if o.metadata.name == name and o.status.conditions and any(c.type == "Complete" for c in o.status.conditions):
            return

def wait_for_daemonset_to_be_available(name: str, namespace: str, tries: int = 18, sleep_time: float = 5.0):
    config.load_kube_config()
...
def wait_for_cronjob_to_run(name: str, namespace:str):
    ...
    w = watch.Watch()
    for event in w.stream(batchapi_client.list_namespaced_cron_job, namespace=namespace):
